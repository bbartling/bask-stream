...
NiagaraFalls is a Niagara 4 runtime module that exposes station data through an authenticated WebSocket API. The goal is to give external applications a practical live-data path for graphics, dashboards, commissioning tools, and integrations without forcing every app to live inside Niagara UI.
...
```text
NiagaraFalls-rt/                 Niagara runtime module source
docs/THIRD_PARTY_API.md          Detailed WebSocket protocol guide
tools/niagarafalls-nav-tree.html Companion guide and demo/test app
tools/niagarafalls-nav-tree-server.mjs
                                 Local helper for the companion app
tools/niagarafalls-live-smoke.mjs
                                 Live station smoke test script
tools/niagarafalls-test.html     Lower-level standalone test harness
tools/niagarafalls-test-snippet.js
...
2. Install the compiled NiagaraFalls module jar on the station host.
3. Restart Niagara so the module is available.
4. Add the NiagaraFalls service to the station.
5. Verify the health endpoint responds after station login:

```text
GET https://<station>/falls/health
```

6. Create or choose a Niagara user with only the permissions your external application needs.
...
1. Authenticate to Niagara and open the WebSocket at `/falls`.
...
| Alarms | `read_alarms`, `subscribe_alarms`, `unsubscribe_alarms` | Load bounded snapshots and maintain client alarm state with event pushes. |
...
- Firewalls and proxies must allow WebSocket upgrade traffic to `/falls`.
...
```text
tools/niagarafalls-nav-tree.html
```
...
```bash
node tools/niagarafalls-nav-tree-server.mjs --port=8787
```
...
1. Start the helper with `node tools/niagarafalls-nav-tree-server.mjs --port=8787`.
...
- `NiagaraFalls-rt/DEVELOPMENT-NOTES.md` captures implementation notes and follow-up candidates.
